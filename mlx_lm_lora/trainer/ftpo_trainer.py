...
def ftpo_loss(
    ...
    rejected_logits = mx.take_along_axis(
        policy_logits, rejected_ids[:, None], axis=-1
    )
    delta = chosen_logits - rejected_logits
    weights = mx.clip(
        (clip_epsilon_logits - delta) / clip_epsilon_logits, 0.0, 1.0
    ) * chosen_mask
    ...
    rejected_diff_sq = mx.take_along_axis(
        diff_sq, rejected_ids[:, None], axis=-1
    )
    target_count = chosen_mask.sum() + policy_logits.shape[0]
    non_target_count = policy_logits.size - target_count
    non_target_sq = (
        diff_sq.sum()
        - (chosen_diff_sq * chosen_mask).sum()
        - rejected_diff_sq.sum()
    )
    ...
    rejected_excess_sq = mx.take_along_axis(
        excess_sq, rejected_ids[:, None], axis=-1
    )
    mse_target = (
    ...
    metrics = {
        ...
        "margin_win": (
            ((delta >= clip_epsilon_logits) & active).astype(mx.float32).sum()
            / denom
        ),
...
def iterate_ftpo_batches(dataset, batch_size, max_seq_length, train=False):
    if len(dataset) < batch_size:
        raise ValueError(f"FTPO dataset must contain at least batch_size={batch_size} rows")
    ...
    while True:
        batch_order = np.random.permutation(len(batches)) if train else range(len(batches))
        for batch_index in batch_order:
            ...
            if any(length == 0 or length > max_seq_length for length in lengths):
                raise ValueError("FTPO prompts must be non-empty and fit max_seq_length")
            width = max(lengths)
            ...
            yield tuple(map(mx.array, (prompts, np.asarray(lengths), chosen, chosen_mask, rejected)))
        if not train:
            break
...
def _batch_loss(model, ref_model, batch, args):
    ...
    return ftpo_loss(
        policy_logits, reference_logits, chosen, chosen_mask, rejected,
        args.lambda_mse_target, args.tau_mse_target, args.lambda_mse,
        args.clip_epsilon_logits,
    )


def evaluate_ftpo(model, ref_model, dataset, batch_size, num_batches, max_seq_length, args):
    model.eval()
    totals = None
...
def train_ftpo(
    ...
    value_and_grad = nn.value_and_grad(model, lambda m, batch: _batch_loss(m, ref_model, batch, args))
    ...
    model.train()
    iterator = iterate_ftpo_batches(train_dataset, args.batch_size, args.max_seq_length, train=True)
    ...
    for iteration in pbar:
        if val_dataset and (iteration == 1 or iteration % args.steps_per_eval == 0):
            val_loss, val_metrics = evaluate_ftpo(model, ref_model, val_dataset, args.batch_size, args.val_batches, args.max_seq_length, args)
            if training_callback:
                training_callback.on_val_loss_report({"iteration": iteration, "val_loss": val_loss, **{f"val_{k}": v for k, v in val_metrics.items()}})
            model.train()
        loss, _, metrics, accumulated_grad = step(
            next(iterator), accumulated_grad,
            iteration % args.gradient_accumulation_steps == 0,
        )
        loss_sum += loss
        metric_sums = metrics if metric_sums is None else {k: metric_sums[k] + v for k, v in metrics.items()}
        report_steps += 1
        ...
        if iteration % args.steps_per_report == 0 or iteration == args.iters:
            ...
            avg_loss = mx.distributed.all_sum(loss_sum).item() / (report_steps * world.size())
            avg_metrics = {k: mx.distributed.all_sum(v).item() / (report_steps * world.size()) for k, v in metric_sums.items()}
            if rank == 0:
                tqdm.write(f"Iter {iteration}: loss {avg_loss:.4f}, chosen_win {avg_metrics['chosen_win']:.3f}, margin_win {avg_metrics['margin_win']:.3f}, it/s {report_steps / elapsed:.3f}")
            if training_callback:
                training_callback.on_train_loss_report({"iteration": iteration, "train_loss": avg_loss, **{f"train_{k}": v for k, v in avg_metrics.items()}})
            loss_sum, metric_sums, report_steps, start = mx.array(0.0), None, 0, time.perf_counter()
        if iteration % args.steps_per_save == 0:
            mx.save_safetensors(str(args.adapter_file), dict(tree_flatten(model.trainable_parameters())))
