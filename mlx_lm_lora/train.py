...
def build_parser():
    ...
    parser.add_argument("--model", type=str, help="The path to the local model directory or Hugging Face repo.")
    parser.add_argument("--load-in-4bits", action="store_true", help="Load the model in 4-bit quantization.", default=None)
    parser.add_argument("--load-in-6bits", action="store_true", help="Load the model in 6-bit quantization.", default=None)
    parser.add_argument("--load-in-8bits", action="store_true", help="Load the model in 8-bit quantization.", default=None)
    parser.add_argument("--train", action="store_true", help="Do training", default=None)
    parser.add_argument("--data", type=str, help="Directory with {train, valid, test}.jsonl files or the name of a Hugging Face dataset")
    parser.add_argument("--train-type", type=str, choices=["lora", "dora", "full"], help="Type of fine-tuning to perform: lora, dora, or full.")
    parser.add_argument("--train-mode", type=str, default="sft", choices=["sft", "dpo", "cpo", "orpo", "grpo", "online_dpo", "xpo", "rlhf_reinforce", "ppo"], help="Training mode")
    parser.add_argument("--optimizer", type=str, choices=["adam", "adamw", "muon"], default=None, help="Optimizer to use for training")
    parser.add_argument("--mask-prompt", action="store_true", help="Mask the prompt in the loss when training", default=None)
    parser.add_argument("--num-layers", type=int, help="Number of layers to fine-tune. Default is 16, use -1 for all.")
    ...
    parser.add_argument("--val-batches", type=int, help="Number of validation batches, -1 uses the entire validation set.")
    parser.add_argument("--learning-rate", type=float, help="Optimizer learning rate.")
    parser.add_argument("--steps-per-report", type=int, help="Number of training steps between loss reporting.")
    parser.add_argument("--steps-per-eval", type=int, help="Number of training steps between validations.")
    parser.add_argument("--resume-adapter-file", type=str, help="Load path to resume training from the given fine-tuned weights.")
    parser.add_argument("--adapter-path", type=str, help="Save/load path for the fine-tuned weights.")
    parser.add_argument("--save-every", type=int, help="Save the model every N iterations.")
    parser.add_argument("--test", action="store_true", help="Evaluate on the test set after training", default=None)
    parser.add_argument("--test-batches", type=int, help="Number of test set batches, -1 uses the entire test set.")
    parser.add_argument("--max-seq-length", type=int, help="Maximum sequence length.")
    parser.add_argument("-c", "--config", type=str, help="A YAML configuration file with the training options")
    parser.add_argument("--grad-checkpoint", action="store_true", help="Use gradient checkpointing to reduce memory use.", default=None)
    parser.add_argument("--wandb", type=str, default=None, help="WandB project name to report training metrics. Disabled if None.")
    parser.add_argument("--seed", type=int, help="The PRNG seed")
    parser.add_argument("--fuse", action="store_true", help="Fuse and save the trained model.", default=None)
    parser.add_argument("--beta", type=float, help="Temperature parameter for ORPO training.", default=0.1)
    parser.add_argument("--reward-scaling", type=float, help="Reward scaling factor for ORPO training, not implemented.", default=1.0)
    parser.add_argument("--dpo-cpo-loss-type", type=str, help="DPO loss type: 'sigmoid', 'hinge', 'ipo', or 'dpop'.", choices=["sigmoid", "hinge", "ipo", "dpop"], default="sigmoid")
    parser.add_argument("--delta", type=float, help="Delta parameter for DPOP loss type.", default=50.0)
    parser.add_argument("--reference-model-path", type=str, help="Path to reference model weights. If None, uses the same model.", default=None)
    parser.add_argument("--judge", type=str, help="Judge to use can be a model ID or 'human'.", default="mlx-community/Josiefied-Qwen2.5-7B-Instruct-abliterated-v2-4-bit")
    parser.add_argument("--alpha", type=list[float], help="Judge to use can be a model ID or 'human'.", default=[1e-5])
    parser.add_argument("--group-size", type=int, help="Number of generations.", default=4)
    parser.add_argument("--max-completion-length", type=int, help="Maximum length of the prompt.", default=512)
    parser.add_argument("--epsilon", type=float, help="The Epsilon for numerical stability.", default=1e-4)
    parser.add_argument("--temperature", type=float, help="Temperature for sampling.", default=1.0)
    parser.add_argument("--reward-weights", type=str, help="Weights for each reward function.", default=None)
    parser.add_argument("--reward-functions", type=str, help="Comma-separated list of reward function names to use.", default=None)
    parser.add_argument("--reward-functions-file", type=str, help="Path to a Python file containing custom reward functions.", default=None)
    parser.add_argument("--list-reward-functions", action="store_true", help="List all available reward functions and exit")
    parser.add_argument("--grpo-loss-type", type=str, help="GRPO loss type: 'grpo', 'bnpo', or 'dr_grpo'.", choices=["grpo", "bnpo", "dr_grpo"], default="grpo")
    parser.add_argument("--epsilon-high", type=float, help="Upper-bound epsilon value for clipping.", default=None)
    parser.add_argument("--importance-sampling-level", type=str, choices=["token", "sequence", None], default=None, help="Level of importance sampling to use.")
    return parser
...
def train_model(args, model: nn.Module, tokenizer, reference_model: nn.Module = None, judge_model: nn.Module = None, judge_tokenizer = None, train_set: CacheDataset = None, valid_set: CacheDataset = None, training_callback: TrainingCallback = None):
    ...
    if args.num_layers > len(model.layers):
        raise ValueError(f"Requested to train {args.num_layers} layers but the model only has {len(model.layers)} layers.")
    
    if args.train_type == "full":
        for l in model.layers[-max(args.num_layers, 0):]:
            l.unfreeze()
    elif args.train_type in ["lora", "dora"]:
        linear_to_lora_layers(model, args.num_layers, args.lora_parameters, use_dora=(args.train_type == "dora"))
    else:
    ...
    if args.train_mode == "orpo":
        train_orpo(
            ...
            args=ORPOTrainingArgs(
                ...
                steps_per_save=args.save_every, adapter_file=adapter_file,
        ...
        train_args_kwargs = {
            ...
            "steps_per_save": args.save_every, "adapter_file": adapter_file,
            "max_seq_length": args.max_seq_length, "grad_checkpoint": args.grad_checkpoint,
            "beta": args.beta, "reference_model_path": args.reference_model_path,
            "gradient_accumulation_steps": args.gradient_accumulation_steps,
            "judge": args.judge, "max_completion_length": args.max_completion_length,
        }
        
        if args.train_mode in ["online_dpo", "xpo"]:
            train_args_kwargs.update({"loss_type": args.dpo_cpo_loss_type, "delta": args.delta})
        if args.train_mode == "ppo":
            train_args_kwargs.update({"loss_type": args.dpo_cpo_loss_type, "delta": args.delta, "epsilon": args.epsilon, "temperature": args.temperature})
        ...
        train_grpo(
            ...
            args=GRPOTrainingArgs(
                ...
                steps_per_report=args.steps_per_report, steps_per_eval=args.steps_per_eval,
                steps_per_save=args.save_every, adapter_file=adapter_file,
                max_seq_length=args.max_seq_length, max_completion_length=args.max_completion_length,
                grad_checkpoint=args.grad_checkpoint, beta=args.beta, group_size=args.group_size,
                epsilon=args.epsilon, epsilon_high=args.epsilon_high,
                reference_model_path=args.reference_model_path, temperature=args.temperature,
                gradient_accumulation_steps=args.gradient_accumulation_steps,
